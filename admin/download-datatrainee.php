...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $query = "SELECT * FROM karyawan_magang WHERE nama_lengkap LIKE ? ORDER BY ba_cabang ASC";
    $stmt = $conn->prepare($query);
    $searchTerm = "%" . $search . "%";
    $stmt->bind_param("s", $searchTerm);
} else {
    $query = "SELECT * FROM karyawan_magang ORDER BY ba_cabang ASC";
    $stmt = $conn->prepare($query);
}
$stmt->execute();
